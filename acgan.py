...
class acgan():
    ...
    def discriminator_model(self):
        ...
        x = layers.Flatten()(x)
        x1 = layers.Dense(1)(x)
        x2 = layers.Dense(self.class_dim)(x)

        model = keras.Model(inputs=image, outputs=[x1,x2])
        return model
    
    def discriminator_loss(self,real_output,fake_output,real_pred,label):
        ...
        total_loss = real_loss + fake_loss + real_label
        return total_loss
    ...
    def train_step(self,images,labels):
        ...
        with tf.GradientTape() as gen_tape , tf.GradientTape() as disc_tape:
            ...
            real_output,real_pred = self.discriminator( images ,training=True )
            fake_output,fake_pred = self.discriminator( generated_images ,training=True )

            gen_loss = self.generator_loss(fake_output,fake_pred,labels)
            disc_loss = self.discriminator_loss(real_output,fake_output,real_pred,labels)
